...
# ------------------- Pagina 2 --------------------------
    ...
    try:
        ...
        ev_data["month"] = ev_data["start_time"].dt.strftime("%b %Y")
        ...
        phase_options = ["Alle"] + sorted(ev_data["n_phases"].dropna().unique())
        ...
        fig_hour = px.bar(hourly_counts, x="hour", y="Aantal laadsessies")
        ...
        fig_energy = px.bar(energy_by_month, x="month", y=energy_col)
        fig_energy.update_xaxes(title_text="Maand")
        fig_energy.update_yaxes(title_text="Totaal geladen energie (kWh)")
        ...
        avg_duration = ev_filtered.groupby("month")["session_duration"].mean().reset_index().sort_values("month")
        fig_duration = px.line(avg_duration, x="month", y="session_duration", markers=True)
        fig_duration.update_xaxes(title_text="Maand")
        fig_duration.update_yaxes(title_text="Gemiddelde sessieduur (uren)")
        st.plotly_chart(fig_duration, use_container_width=True)

        # --- Boxplot energie per sessie ---
        st.subheader("Verdeling van geladen energie per sessie per maand")
        fig_box = px.box(ev_filtered, x="month", y=energy_col, points="all")
        fig_box.update_xaxes(title_text="Maand")
        fig_box.update_yaxes(title_text="Energie per sessie (kWh)")
